...
app.post("/read", (req, res) => {
  ...
  if( start==0 ) res.json( {messages: bbs, name0: name0, gn: gn});
  else res.json( {messages: bbs.slice( start ), name0: name0, gn: gn});
});
...
app.post("/search_post", (req, res) => {
  const search = req.body.name0;
  console.log( [search] );
  // 本来はここでDBMSに保存する
  name0.push( { name0: search} );
  res.json( {number: name0.length } );
});

app.post("/good_post", (req, res) => {
  const good = req.body.gn;
  console.log( [good] );
  // 本来はここでDBMSに保存する
  gn.push( { gn: good} );
  res.json( {number: gn.length } );
});
